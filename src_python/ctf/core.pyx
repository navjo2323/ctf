...
cdef extern from "ctf.hpp" namespace "CTF":
    ...
    cdef void TTTP_Tucker_ "CTF::TTTP_Tucker"[dtype](Tensor[dtype] * T, Tensor[dtype] ** mat_list, Tensor[dtype] * core, bool aux_mode_first)
    cdef void TTMC_ "CTF::TTMC"[dtype](Tensor[dtype] * T, Tensor[dtype] ** mat_list, Tensor[dtype] * core, bool aux_mode_first)
    cdef void MTTKRP_ "CTF::MTTKRP"[dtype](Tensor[dtype] * T, Tensor[dtype] ** mat_list, int mode, bool aux_mode_first)
    cdef void MTTKRP_Tucker_ "CTF::MTTKRP_Tucker"[dtype](Tensor[dtype] * T, Tensor[dtype] ** mat_list, Tensor[dtype] * core, int mode, bool aux_mode_first)
...
def TTTP_Tucker(tensor A, mat_list,tensor C):
    """
    TTTP(A, mat_list,core)
    Compute updates to entries in tensor A based on matrices in mat_list (tensor times tensor products)
    This routine is generally much faster then einsum when A is sparse.
        ...
        if mat_list[i] is not None:
            ...
            if mat_list[i].ndim == 1:
                if k != -1:
                    raise ValueError('CTF PYTHON ERROR: mat_list must contain only vectors or only matrices')
                if mat_list[i].shape[0] != A.shape[i]:
                    raise ValueError('CTF PYTHON ERROR: input vector to TTTP_TUCKER does not match the corresponding tensor dimension')
                        #exp = exp*mat_list[i].i(s[i])
            else:
                if mat_list[i].ndim != 2:
                    raise ValueError('CTF PYTHON ERROR: mat_list operands has invalid dimension')
            # Add for core dimensions should match
                if k == -1:
                    if mat_list[i].shape[1] != C.shape[i]:
                        raise ValueError('CTF PYTHON ERROR: input matrix to TTTP TUCKER does not match the corresponding Core dimension')
    B = tensor(copy=A)
    if A.dtype == np.float64:
        TTTP_Tucker_[double](<Tensor[double]*>B.dt,tsrs,<Tensor[double]*>C.dt,1)
    else:
        raise ValueError('CTF PYTHON ERROR: TTTP does not support this dtype')
    free(modes)
    free(tsrs)
...
def TTMC(tensor A, mat_list,tensor C):
    """
    TTTP(A, mat_list,core)
    Compute updates to entries in tensor A based on matrices in mat_list (tensor times tensor products)
    This routine is generally much faster then einsum when A is sparse.
    ...
    t_tttp.start()
    if len(mat_list) != A.ndim:
        raise ValueError('CTF PYTHON ERROR: mat_list argument to TTMC must be of same length as ndim')
    ...
    D = tensor(copy=C)
    if A.dtype == np.float64:
        TTMC_[double](<Tensor[double]*>B.dt,tsrs,<Tensor[double]*>D.dt,1)
    else:
        raise ValueError('CTF PYTHON ERROR: TTMC does not support this dtype')
    free(modes)
    free(tsrs)
    t_tttp.stop()
    return D
...
def MTTKRP_Tucker(tensor A, mat_list, tensor C, mode):
    """
    Solve_Factor_Tucker(A, mat_list,core,R, mode,regu)
    solves for a factor matrix parallelizing over rows given rhs, sparse tensor and list of factor matrices
    eg. for mode=0 order 3 tensor Computes LHS = einsum("ijk,jr,jz,kr,kz->irz",T,B,B,C,C) and solves each row with rhs
    ...
    B = tensor(copy=A)
    if A.dtype == np.float64:
        MTTKRP_Tucker_[double](<Tensor[double]*>B.dt,tsrs,<Tensor[double]*>C.dt,A.ndim-mode-1,1)
    else:
        raise ValueError('CTF PYTHON ERROR: Solve_Factor_Tucker does not support this dtype')
    free(tsrs)
    t_solve_factor_t.stop()
